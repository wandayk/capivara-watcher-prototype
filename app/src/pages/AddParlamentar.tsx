...
import { AddParlamentarLayout } from "../components/layout/AddParlamentarLayout";
...
const stateNameToUF: { [key: string]: string } = {
  Acre: "AC",
  Alagoas: "AL",
  Amapá: "AP",
  Amazonas: "AM",
...
export function AddParlamentar() {
  const { isParlamentarAdded, addParlamentar, selectParlamentar } =
    useParlamentares();
  ...
  // Implementação do JavaScript para eventos de clique e hover nos estados
  useEffect(() => {
    ...
    const handleStateClick = (event: Event) => {
      ...
      if (name && stateNameToUF[name]) {
        ...
        // Se já está selecionado, desseleciona; caso contrário, seleciona
        setSelectedUF((currentUF) => {
          const newUF = currentUF === uf ? null : uf;

          // Atualizar classes CSS para destacar o estado selecionado
          const allStates = document.getElementsByClassName("estado");
    ...
    const handleStateMouseEnter = (event: Event) => {
      ...
      if (name && code && stateNameToUF[name]) {
        ...
        // Contar deputados e senadores deste estado
        const deputados = parlamentares.filter(
          (p) => p.uf === uf && p.tipo === "deputado",
        ).length;
        const senadores = parlamentares.filter(
          (p) => p.uf === uf && p.tipo === "senador",
        ).length;

        setHoveredState({ name, code, uf, deputados, senadores });
      }
    };
    ...
    for (let i = 0; i < states.length; i++) {
      states[i].addEventListener("click", handleStateClick);
      states[i].addEventListener("mouseenter", handleStateMouseEnter);
      states[i].addEventListener("mouseleave", handleStateMouseLeave);
    }
    ...
    return () => {
      for (let i = 0; i < states.length; i++) {
        states[i].removeEventListener("click", handleStateClick);
        states[i].removeEventListener("mouseenter", handleStateMouseEnter);
        states[i].removeEventListener("mouseleave", handleStateMouseLeave);
      }
      document.removeEventListener("mousemove", handleMouseMove);
    };
  }, [parlamentares]); // Atualiza quando a lista de parlamentares mudar

  // Sincronizar visual do mapa com selectedUF
  useEffect(() => {
    const allStates = document.getElementsByClassName("estado");
    ...
    // Se há um UF selecionado, marcar o estado correspondente
    if (selectedUF) {
      // Encontrar o nome do estado pela sigla
      const stateName = Object.keys(stateNameToUF).find(
        (key) => stateNameToUF[key] === selectedUF,
      );
  ...
  return (
    <AddParlamentarLayout>
      {/* Tooltip do mapa */}
      {hoveredState && (
        <div
          className="fixed z-50 pointer-events-none bg-gray-900 text-white px-3 py-2 rounded-lg shadow-lg text-sm min-w-45"
          style={{
          ...
          <div className="text-xs text-gray-300 mt-1 pt-1 border-t border-gray-700">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-blue-400">
                {hoveredState.deputados}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Senadores:</span>
              <span className="font-semibold text-green-400">
                {hoveredState.senadores}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="h-full flex flex-col lg:flex-row overflow-hidden bg-gray-400/10">
        {/* Left Side - Map */}
        <div className="lg:w-3/5 p-0 lg:p-6 overflow-y-auto relative">
        ...
        <div className="bg-white shadow-md lg:w-2/5 flex flex-col border-t lg:border-t-0 lg:border-l border-light-border dark:border-dark-border">
          {/* Filters */}
          <div className="flex justify-between">
            <h2 className="text-2xl p-8 font-bold">Filtro</h2>
          ...
          <div className="px-8 shadow-[0_5px_5px_-5px_rgba(0,0,0,0.3)] border-b border-light-border dark:border-dark-border space-y-4">
            {/* Search */}
            <Input
            ...
            <div className="flex gap-8">
              ...
              <div className="flex w-full items-center gap-4">
                ...
                <select
                  ...
                  className="w-full text-xs font-semibold leading-0 flex items-center h-8 px-4 py-2 rounded-lg bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border text-light-text dark:text-dark-text"
                >
